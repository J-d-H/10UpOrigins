...
class BlaBox {
	...
	static public function renderAll(g: Graphics): Void
	{
		for (box in boxes)
		{
			box.render(g);
	...
	public function render(g: Graphics, x: Float = -1, y: Float = -1): Void {
		...
		if (x < 0) {
			x = (speaker == null) ? (0.5 * (Main.width - width)) : sx - 0.3 * width;
			
			if (x + width > Main.width) {
				x -= 30 + x + width - Main.width;
			}
			if (x < 0) {
				x = 30;
			}
		...
		if (y < 0) {
			y = (speaker == null) ? (0.3 * (Main.height - height)) : sy - 30 - height;
			if (y < 0) {
